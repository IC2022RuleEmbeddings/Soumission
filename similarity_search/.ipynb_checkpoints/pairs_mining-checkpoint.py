        ...
        if min_value >= distance_threshold:
            ...
            while min_value < distance_threshold:
                ...
                if mode=='mixed': # squared matrix - we update 2 values
                    df_to_numpy[position[0][0]] = [distance_threshold]
                    df_to_numpy[:,position[1][0]] = [distance_threshold]
...
def get_pairs_from_matrix_and_proportion(df,proportion=0.05,mode='mixed'):
    """
    ...
    df__ = copy.deepcopy(df)
    df_to_numpy = df__.to_numpy()
    number_possible_pairs = len(df__)*(len(df__)-1)/2
    pairs_closer_instances = []
    number_to_reach = number_possible_pairs*proportion
